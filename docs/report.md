...
网页读取 `web/demo-data.json`。该文件由 `scripts/prepare_dataset.py` 从真实数据和 ArangoDB 建模结果导出，适合 GitHub Pages 静态演示。由于 GitHub Pages 是静态网页，前端只嵌入少量问题的图结构用于交互演示；原始 `train/dev/test` 全量数据已保存在仓库中，且全部 192606 条记录均已生成 ArangoDB JSONL 导入文件。页面同时展示全量原始记录数、全量索引记录数和前端演示子集规模。
